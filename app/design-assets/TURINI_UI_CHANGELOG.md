...
## 2026-09-23 — AI 코치·퀴즈 캐릭터 정리, 말투 통일

### 고친 파일
- `app/page.tsx`
  - AI 코치 탭 캐릭터: 저장된 꾸미기 대신 `BASIC_DISPLAY_CUSTOMIZATION` → 모자·안경이 붙지 않아요.
  - 퀴즈 풀이 전: 스프라이트 대신 `QuizThinkingTurini`(새싹 + 기본 초록 가방 + 전구) 사용. 문제마다 새로 등장해요.
  - 퀴즈 정답·오답 팻말: `BASIC_DISPLAY_CUSTOMIZATION` → 모자·안경·목장식 없이 새싹만 보여요.
  - 포트폴리오 결과의 개조식 문구(…아님/…안 함/…없음/산출 불가)를 ~해요/~이에요체로 바꿨어요.
  - 상세 분석의 분산효과 상태가 영어 코드(valid 등)로 보이던 것을 한국어 문장으로 바꿨어요.
- `app/turini-avatar.tsx` — `QuizThinkingTurini` 컴포넌트 추가
- `app/turini-character.css` — 등장(톡 튀어나옴) · 숨쉬기 · 고개 갸웃 · "아하!" 전구 후광/별 애니메이션, 애니메이션 줄이기 대응
- `tests/public-release.test.mjs` — 회귀 테스트 2개

### 구현하지 못한 것
- 정답·오답 팻말 12프레임에는 가방이 그려져 있지 않아요. 가방을 몸 뒤에 겹쳐 봤지만 정면 자세라 몸에 완전히 가려져 보이지 않았어요.
  팻말 장면에도 가방을 넣으려면 **가방을 멘 채 체크/X 팻말을 든 12프레임 이미지**(또는 어깨끈 레이어)가 필요해요.
- 생각하는 투리니 PNG는 한 장짜리라 눈 깜박임은 넣지 못했어요. 넣으려면 눈 감은 얼굴 레이어가 필요해요.
