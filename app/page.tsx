...
/** 분산효과 계산 상태를 사용자에게 보여 줄 친근한 문장으로 바꿉니다. */
const DIVERSIFICATION_STATUS_TEXT: Record<PortfolioResult["diversificationStatus"], string> = {
  valid: "정상적으로 계산했어요",
  zero_reference: "비교할 기준 변동성이 0이에요",
  invalid: "값을 계산할 수 없어요",
};
      ...
      <main className="quiz-stage">
        <section className="quiz-phone" data-question-id={question.id} data-concept-id={conceptKey(question)} data-review-kind={question.reviewKind || "new"}>
          ...
          <div className={`quiz-mascot ${answered ? "answered" : "waiting"}`}>
            {answered
              ? <TuriniAvatar customization={BASIC_DISPLAY_CUSTOMIZATION} motion={answerCorrect ? "correct" : "wrong"} className="turini-quiz" replayKey={`${question.id}-${session.index}`} holdLast />
              : <QuizThinkingTurini key={`${question.id}-${session.index}`} className="turini-quiz turini-quiz--thinking" />}
        ...
        <section className="app-main">
          ...
          {view === "portfolio" && (
            <div className="screen portfolio-screen">
              ...
              <section className="portfolio-options card-block"><div><label>입력 구분<select value={portfolioInputMode} onChange={(event) => setPortfolioInputMode(event.target.value as PortfolioInputMode)}><option value="actual">실제 자산</option><option value="practice">가상 연습</option></select><em>분석 기록의 성격만 구분하고 계산식은 같아요.</em></label><label>투자 목적 <small>(기록용)</small><select value={goal} onChange={(event) => setGoal(event.target.value)}><option>장기 자산 증식</option><option>주택·목돈 마련</option><option>은퇴 준비</option><option>단기 여유자금 운용</option></select><em>현재 판정·조정 계산에는 반영되지 않아요.</em></label><label>투자 기간<select value={horizon} onChange={(event) => { setHorizon(event.target.value); setPortfolioResult(null); }}>{PORTFOLIO_HORIZONS.map((option) => <option key={option}>{option}</option>)}</select></label></div></section>
